...
public sealed class BH0002FieldsShouldBeAtTheTopAnalyzer : BHAnalyzer {
    ...
    private static readonly LocalizableResourceString Title = new(nameof(Resources.BH0002AnalyzerTitle), Resources.ResourceManager,
        typeof(Resources));

    private static readonly LocalizableResourceString MessageFormat = new(nameof(Resources.BH0002AnalyzerMessageFormat), Resources.ResourceManager,
        typeof(Resources));

    private static readonly LocalizableResourceString Description = new(nameof(Resources.BH0002AnalyzerDescription), Resources.ResourceManager,
        typeof(Resources));

    private static readonly DiagnosticDescriptor Rule = new(DiagnosticId, Title, MessageFormat, Category, DiagnosticSeverity.Warning, false,
        Description, $"{BaseUrl}BH0002");
    ...
    private static void SemanticModelAction(SemanticModelAnalysisContext context) {
        ...
        foreach (var variable in model.SyntaxTree.GetRoot(token).DescendantNodesAndSelf().OfType<VariableDeclarationSyntax>()
            ...
            var members = type.GetMembers().Where(m =>
                ...
                && !m.DeclaringSyntaxReferences
                    .Any(sr => sr.GetSyntax(context.CancellationToken) is ClassDeclarationSyntax or StructDeclarationSyntax)).ToArray();
