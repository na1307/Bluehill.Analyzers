...
public sealed class EnumExtensionsGenerator : IIncrementalGenerator {
    ...
    private static void Generate(SourceProductionContext context, EnumInfo info) {
        ...
        sb.Append(accessibilityKeyword).Append(" static class ").Append(GetEscapedName(typeName)).AppendLine("Extensions {")
            .Append("    ").Append(accessibilityKeyword).Append(" static string ToStringFast(this ").Append(typeName)
        ...
        if (isFlags) {
            sb.AppendLine().Append("    ").AppendLine("[global::System.Runtime.CompilerServices.MethodImplAttribute(256)]").Append("    ")
                .Append(accessibilityKeyword).Append(" static bool HasFlagFast(this ").Append(typeName).Append(" value, ")
